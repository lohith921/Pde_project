...
booloperator<(constEdge&other)const{
if(vmin<other.vmin)returntrue;
if(vmin>other.vmin)returnfalse;
if(vmax<other.vmax)returntrue;
returnfalse;
}
};
...
EdgegetEdge(intk){
returnEdge(V[k],V[(k+1)%3]);
}
boolinCircle(Vertex*c);
Vertexcentroid();
};

voidcomputeAdjacencies(std::vector<Face*>&cavity){
std::map<Edge,std::pair<int,Face*>>edgeToFace;
for(intiFace=0;iFace<cavity.size();iFace++){
for(intiEdge=0;iEdge<3;iEdge++){
Edgeedge=cavity[iFace]->getEdge(iEdge);
std::map<Edge,std::pair<int,Face*>>::iteratorit=
...
voiddelaunayCavity(Face*f,Vertex*v,std::vector<Face*>&cavity,
...
elsedelaunayCavity(f->F[iNeigh],v,cavity,bnd,otherSide);
}
}
