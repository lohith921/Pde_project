...
/* CircumCircle() : Returns true if the point (xp,yp) is inside the circumcircle made up
   of the points (x1,y1), (x2,y2), (x3,y3). The logic is independant of orientation of nodes of triangle.   
...
/* Triangulate() : Triangulation subroutine, Takes as input NV vertices in array pnode.
   		   Returns a list of ntri triangular faces in the array v. These triangles are 
   		   arranged in a consistent clockwise order. The triangle array 'v' should be malloced to 3 * nv.
	   	   The vertex array pnode must be big enough to hold 3 more points(the bounding triangle).
*/
...
// Outputtriangle is used to print out triangle nodes
...
int main(){
	...
	inputFile.close();
  	...
  	outFile.open(fname + ".ele",ios::out);
  	outFile << ntri << std::endl;
  	for (i=0;i < ntri; i++)
  		outFile << (i+1) << " " << v[i].p1 << " " << v[i].p2 << " " << v[i].p3 << sendl;
  	outFile.close();
